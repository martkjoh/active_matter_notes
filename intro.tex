...
\begin{itemize}
    ...
    \begin{figure}[!htb]
        ...
        \caption{Motion at a microscopic level: to the left, microswimmers move using flagella. To the right, a cell moves by polymerizing actine filaments at its front.}
        \label{fig: label}
    \end{figure}
...
\noindent 
    ...
    Here, we assume for simplicity that $\theta = 0$ at time $t = 0$.
    \begin{enumerate}
        ...
        \end{equation*}
        \item In the case where $\theta(t)$ diffuses with a diffusion coefficient $D_r$, show that the associated distribution is a Gaussian with zero mean and variance $2 D_r t$.
...
%\todo[noinline]{Draw contour, explain BM: Not necessary, one can simply express the indefinite integral in terms of arctan.}
%
\begin{align}
